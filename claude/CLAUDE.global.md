...
|------|-------|--------|-------------|
| **Advisor / architect** — decompose the task, design the plan, assign each chunk a model | **Fable 5** → Opus 4.8 (1M) if Fable unavailable | low/med generic · high/xhigh when hard | subagent `advisor`, `/plan`, or Workflow advisor stage |
...
1. **Advise** — Fable produces the plan and the chunk→model assignment. (`/plan` or `advisor` agent.)
